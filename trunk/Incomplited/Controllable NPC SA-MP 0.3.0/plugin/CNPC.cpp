...
CNPC::CNPC(int id,char* name)
{
	...
	pHooks->ClientConnect(pSaMp->players,id,name,1);
...
void CNPC::PutInVehicle(int vehicleid,int seat)
{
	...
	if(pSaMp->vehicles->vehicle[vehicleid])
	{
		if(seat == 0)
		{
			...
			local_class->InCarSync.TrailerId = 0xFFFF;
			local_class->InCarSync.Unique = 0.0;
...
{
	float fix = (360.f - (angle - 90.f)) * (float)(M_PI / 180.f);
	Quaternion quat;
...
{
	local_class->InCarSync.Unique = unique;
}

float CNPC::GetVehicleUnique()
{
	return local_class->InCarSync.Unique;
}

void CNPC::SetVehicleTrailer(int trailer)
{
	local_class->InCarSync.TrailerId = static_cast<unsigned char>(trailer);
}

int CNPC::GetVehicleTrailer()
{
	int tr = static_cast<int>(local_class->InCarSync.TrailerId);
	return tr;
}

void CNPC::SetPassangerDriveBy(int state)
{
	if(state == 1) local_class->PassangerSync.Seat = 129;
	else local_class->PassangerSync.Seat = local_class->VehicleSeat;
}

int CNPC::GetPassangerDriveBy()
{
	if(local_class->PassangerSync.Seat == 129) return 1;
	return 0;
}
